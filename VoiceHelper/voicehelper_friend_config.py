...
if LANGUAGE == 'en':
    ...
    # Быстрая перемотка. Переход к треку под указанным номером (например, "трек 3") или
    # к указанному времени (например 20 секунд)
...
def number_greater_len_pl(len_playlist,):
    if LANGUAGE == 'en':
        return ''''''
    elif LANGUAGE == 'ru':
        return f''', в твоем плейлисте сейчас {len_playlist} треков. 
